...
{	
    ...
    // Etap 1 - wysyłanie nazwy pliku i odbieranie potwierdzenia.
    
    FILE* file = open_file(argv[5], "rb");	// otwarcie pliku
    
    fseek(file, 0, SEEK_END);			// przejście na koniec pliku
    long int fileSize = ftell(file);		// ftell zwraca aktualną pozycję w pliku (w tym przypadku rozmiar pliku)
    
    char * fileName = basename(argv[5]);	// tutaj będzie przypisana sama nazwa pliku bez ścieżki
    strcat(fileName, "?");		        // dopisanie do nazwy pliku '?'
    
    long int fileLen = strlen(fileName);	// sprawdzanie dlugosci nazwy pliku
    ...
    printf("Etap 1 - wysylanie nazwy pliku do kompresji...\n");
    write_to(sfd, fileName, fileLen+1);
    printf("Etap 1 - wyslano nazwe pliku do kompresji.\n");
    /* wysylanie nazwy pliku do kompresji */
    ...
    // Etap 2 - wysylanie rozmiaru pliku do kompresji i odebranie potwierdzenia
    
    fseek(file, 0, SEEK_SET);				// powrót na początek pliku

    int lenFileSize = (int)(floor(log10(fileSize)))+2;	// długość rozmiaru pliku (+1, bo log; +1, bo '?' na końcu)
    ...
    printf("Etap 2 - wysylanie rozmiaru...\n");
    write_to(sfd, cFileSize, lenFileSize+1);
    printf("Etap 2 - wyslano rozmiar.\n");
    /* wysylanie rozmiaru pliku do kompresji */
    ...
    // Etap 3 - wysylanie calego pliku do kompresji i odebranie odpowiedzi

    char * fileData = malloc(fileSize);					// alokacja pamięci dla danych z pliku do kompresji
    ...
    printf("Etap 3 - wysylanie pliku...\n");
    write_to(sfd, fileData, fileSize);
    printf("Etap 3 - wyslano plik.\n");
    /* wysylanie calego pliku do kompresji */
    ...
    if(atoi(argv[3])!=1)
    {
    	// Etap 4 - odbieranie nazwy pliku po kompresji i wysylanie odpowiedzi
    
    	char buf[BUF_SIZE] = {0};				// bufor na nazwe i rozmiar pliku po kompresji
    	int bytes =0;						// zmienna pomagająca w pełnym odczycie danych
    
    	/* pełny odczyt nazwy pliku */
    	printf("Etap 4 - pobieranie nazwy pliku zip...\n");
    	while(1){
            bytes += read(sfd, buf+bytes, BUF_SIZE-bytes);
            if(buf[bytes-2] == '?')
            {
            	printf("Etap 4 - pobrano nazwe pliku zip.\n");
            	break;
            }
    	}
    	/* pełny odczyt nazwy pliku */
    
    	fileLen = strlen(buf)-1;				// sprawdzanie dlugosci nazwy pliku po kompresji
    	char* zipName = malloc(fileLen);			// alokacja pamięci na nazwę pliku po kompresji
    	memset(zipName, 0, fileLen);				// czyszczenie zipName z artefaktów
    	for(long int k=0;k<fileLen;k++)	zipName[k] = buf[k];	// przepisywanie nazwy pliku do kompresji bez '?'		
    
    	send_ans(sfd, "4");					// wysylanie potwierdzenia
    	...
    	file = open_file(zipName, "w+");					// tworzenie uchwytu dla pliku po kompresji
    	for (long int j=0; j<fileSize; j++)	fputc((int)fileData[j], file);	// uzupelnianie pliku danymi
    	fclose(file);								// zamykanie pliku
    
    	/* przenoszenie pliku zip do wybranego katalogu */
